...
## Abstract
加密貨幣的出現，帶來了一個有趣的悖論：假名允許罪犯隱藏於顯而易見的地方，而開放的數據則賦予了調查人員更多的權力，並使法庭分析的衆包成爲可能。
據我們所知，這是目前與加密貨幣相關的最大的標記交易數據集。我們分享了使用邏輯斯蒂迴歸（Logistic 迴歸）算法、隨機森林（RF）算法、多層認知器演算法（MLP）以及圖卷積網絡（GCN）算法的變體來預測非法交易的二元分類任務結果。其中圖卷積網絡（GCN）作爲一種新興的獲取關係信息的方法，其具有特殊的意義。
最後考慮到由於現實交易圖的大小及動態性方面的因素，可視化分析和解釋是很難實現的，我們爲此提供了一個簡單的原型，其能夠確定圖形，並觀察模型在檢測非法活動方面的表現。
...
## 7. Summary
總結來說，提出了一些加密貨幣交易鑑證法（特別是對比特幣），以此打擊犯罪活動。向反洗錢社區提供了一個大的、帶有標籤的交易數據集，這類數據以前從未公開過。
我們分享了早期的實驗結果，使用了各種方法，包括圖卷積網絡，並討論了下一步可能的算法改進。我們爲這些數據的可視化提供了一個原型，最重要的是，我們希望以此激勵他人應對反洗錢這一重大問題挑戰，使我們的金融體系更安全、更具包容性。
